...
def create_test_image_frame(container):
    ...
    img = Image.open(dir).resize((400, 400),Image.LANCZOS)
...
def create_info_frame(container):
    ...
    executionTime.grid(column=0, row=8, columnspan=2,sticky='W', pady=(24, 0))
...
def create_main_window():
    ...
    # title
    ttk.Label(mainFrame, text="Recognize Face using Eigenface Method", style="Heading1.TLabel").grid(row=0,column=0,columnspan=3, pady=(0, 60))
    ...
    result_image_frame.grid(column=1, row=2, padx=80, sticky='N')
    ...
    s = ttk.Style()
    s.theme_use('alt')
    s.configure('.', font=("Futura Bk BT", 20), foreground="#2F2D2D")
    s.configure('Heading1.TLabel', font=("Futura Md BT", 36), foreground='#6C56C2')
    s.configure('Subheading.TLabel', font=("Futura Md BT", 28), foreground='#6C56C2')
    s.configure('Subheading02.TLabel', font=("Futura Md BT", 24), foreground='#2F2D2D')
